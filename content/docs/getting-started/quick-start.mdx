...
Open the `src/` folder to see the Step files. Each one exports a `config` (defining triggers and topics) and a `handler` (business logic). The `config.yaml` describes the infrastructure modules that iii sets up to power Motia.
