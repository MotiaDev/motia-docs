...
- **iii** installed — follow the instructions at [iii.dev/docs](https://iii.dev/docs)
...
Open the `src/` folder to see the Step files. Each one exports a `config` (defining triggers and topics) and a `handler` (business logic). The `iii-config.yaml` describes the infrastructure modules that iii sets up to power Motia.

</Step>

<Step>
### 6. Try the iii Console
...
<Cards>
  ...
  <Card title="Build Your First App" href="/docs/getting-started/build-your-first-motia-app">
    Follow a hands-on tutorial building a complete pet store backend from scratch.
  </Card>
