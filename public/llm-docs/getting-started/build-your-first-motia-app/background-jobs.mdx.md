...
You can follow this guide to learn how to build background jobs with Motia step by step, or clone the repository and get started immediately.
...
Check the logs to see both the API call and the background job execution. You'll see:
  ...
  <Tab value="Python">
    ...
    async def handler(input, ctx):
        try:
            ...
            ctx.logger.error('❌ Deletion Reaper failed - import error')
            return

        ctx.logger.info('🔄 Deletion Reaper started - scanning for pets to purge')

        try:
            pets_to_reap = pet_store.find_deleted_pets_ready_to_purge()
            
            if not pets_to_reap:
                ctx.logger.info('✅ Deletion Reaper completed - no pets to purge')
            ...
            for pet in pets_to_reap:
                ...
                if success:
                    ...
                    ctx.logger.info('💀 Pet permanently purged', {
                        'petId': pet['id'],
                        'name': pet['name'],
                    ...
                    ctx.logger.warn('⚠️ Failed to purge pet', {'petId': pet['id'], 'name': pet['name']})

            ctx.logger.info('✅ Deletion Reaper completed', {
            ...
            ctx.logger.error('❌ Deletion Reaper error', {'error': str(error)})
    ```
  </Tab>
  <Tab value="JavaScript">
...
- **triggers** uses the `cron()` helper from `'motia'`
...
The [iii development console](https://iii.dev/docs) provides tools to monitor your background jobs:
