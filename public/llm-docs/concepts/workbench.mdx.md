...
<video controls className="mb-8 w-full rounded-xl" poster="https://assets.motia.dev/images/gifs/v1/4-motia-workbench.gif">
  <source src="https://assets.motia.dev/videos/mp4/site/v1/4-motia-workbench.mp4" type="video/mp4" />
</video>
...
iii Console is where you'll spend most of your time building with Motia. Think of it as your mission control: you can see your entire backend as a visual diagram, test APIs instantly, watch logs stream in real-time, and trace exactly what happens during each request.

![iii Console Overview](./../img/motia-build-your-app.gif)

---

## Getting Started

Fire up the iii Console:
...
- The iii Console interface at `http://localhost:3000`
...
- **Queue Steps** (blue nodes) - background jobs that run when messages are enqueued
...
Click any trace to see the full details in the right panel, including which messages were enqueued and which Steps they triggered.
...
The States tab shows you everything stored with `state.set()`. This is your window into the persistent data your workflows are using. Note that `state.set()` returns `{ new_value, old_value }` so you can track what changed.
...
The **Editor** tab lets you modify state values directly in the iii Console. Edit the JSON, hit **Save Changes**, and your update is persisted immediately. This is super handy for:
...
- **Messages enqueued** - see exactly which messages were enqueued and when
...
Change a Step file, hit save, and the iii Console refreshes automatically. No need to restart the server or reload the page. This makes for an incredibly fast feedback loop.
...
You can override how any Step looks by creating a `.tsx` or `.jsx` file next to your Step file. Use built-in components like `QueueNode`, `ApiNode`, and `CronNode`, or build completely custom React components from scratch.
...
iii Console isn't just a UI. It's a development tool designed to make building backends faster and less painful. Use it to:
